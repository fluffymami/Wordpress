...
class Zakra_Plugin_Install_Helper {
	...
	public function get_button_html( $slug, $settings = array() ) {
		...
		switch ( $state ) {
			...
			case 'deactivate':
				...
				if ( 'themegrill-demo-importer/themegrill-demo-importer.php' === $plugin_link_suffix ) {
					$button .= '<a class="button" href="' . esc_url( admin_url() . 'themes.php?page=demo-importer' ) . '">' . esc_html__( 'Import Demo Now', 'zakra' ) . '</a>';
				} else {
					$button .= '<a  data-redirect="' . esc_url( $redirect ) . '" data-slug="' . esc_attr( $slug ) . '" class="deactivate-now button" href="' . esc_url( $nonce ) . '" data-name="' . esc_attr( $slug ) . '" aria-label="Deactivate ' . esc_attr( $slug ) . '">' . esc_html__( 'Deactivate', 'zakra' ) . '</a>';
				}
	...
	public function enqueue_scripts() {
		...
		wp_localize_script(
			'zakra-plugin-install-helper', 'pagenow',
			array( 'import' )
		);
	}
}
